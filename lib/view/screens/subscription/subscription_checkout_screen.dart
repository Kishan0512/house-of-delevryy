...
class _SubscriptionCheckoutScreenState extends State<SubscriptionCheckoutScreen> {
  ...
  void initState() {
    ...
    if(_isLoggedIn) {
      ...
      Get.find<RestaurantController>().initCheckoutData(widget.product.restaurantId);
      getToken();
    }
  }
            ...
            return GetBuilder<OrderController>(builder: (orderController) {
              ...
              if(restController.restaurant != null) {
                  ...
                  double _dis = (restController.restaurant.discount != null
                      ...
                      ? restController.restaurant.discount.discount : widget.product.discount;
                  String _disType = (restController.restaurant.discount != null
                      && DateConverter.isAvailable(restController.restaurant.discount.startTime, restController.restaurant.discount.endTime))
                      ? 'percent' : widget.product.discountType;
                  _discount = _discount + ((widget.product.price - PriceConverter.convertWithDiscount(widget.product.price, _dis, _disType)));
                  ...
                  Expanded(child: Scrollbar(child: SingleChildScrollView(
                    ...
                    child: Center(child: SizedBox(
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        Text('Delivery Dates (${widget.selectedDate.length})'),
                        Container(
                          height: 30,
                          ...
                          child: ListView(
                            ...
                            children: <Widget>[
                              for(var item in widget.selectedDate )
                                Container(
                                  color : Colors.orange,
                                  padding: EdgeInsets.all(4),
                                  margin : EdgeInsets.all(3),
                                  child: Text(
                                      DateFormat('dd-MM-yyyy').format(item).toString(),
                          ...
                          return Container(
                            ...
                            child: Column(children: [
                              ...
                              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                                ...
                                InkWell(
                                  onTap: (){
                                    Get.toNamed(RouteHelper.getCouponSubscribeRoute(fromCheckout: true)).then((value) => _couponController.text = couponController.checkoutCouponCode);
                                  },
                                  child: Padding(
                                    padding: const EdgeInsets.all(10.0),
                                    child: Row(children: [
                  ...
                  Container(
                    ...
                    child: !orderController.isLoading ? CustomButton(buttonText: 'confirm_order'.tr, onPressed: () {
                      ...
                      if(!_isCashOnDeliveryActive && !_isDigitalPaymentActive && !_isWalletActive) {
                        ...
                        // }
                        List newList = [];
                        for (var name in widget.selectedDate) {
                          newList.add(name.toString().substring(0,10));
                        };
                        ...
                        Map myInput = {
                          ...
                          'noOfDays' : widget.selectedDate.length,
                          'schedule_at' : widget.timeSlot,
                        ...
                        orderController.placeSubscribeOrder(PlaceSubscribeBody(
                          productId: widget.product.id,
                          qty : widget.qty,
                          selectedDate : newList,
                          noOfDays : widget.selectedDate.length,
                          schedule_at : widget.timeSlot.toString(),
                          days : days,
                          fromDate : widget.fromDate.toString(),
                          toDate : widget.toDate.toString(),
                          couponDiscountAmount: Get.find<CouponController>().discount, distance: orderController.distance,
                          couponDiscountTitle: Get.find<CouponController>().discount > 0 ? Get.find<CouponController>().coupon.title : null,
                          scheduleAt: widget.timeSlot.toString(),
                          orderAmount: _total,
                          orderNote: _noteController.text,
                          orderType: orderController.orderType,
                          paymentMethod: orderController.paymentMethodIndex == 0 ? 'cash_on_delivery'
  ...
  void _callbackSubsribe(bool isSuccess, String message, String orderID, double amount) async {
    ...
    if(isSuccess) {
      ...
      if(Get.find<OrderController>().paymentMethodIndex == 0 || Get.find<OrderController>().paymentMethodIndex == 2) {
        ...
        if(GetPlatform.isWeb) {
          ...
          String selectedUrl = '${AppConstants.BASE_URL}/subscribe-payment-mobile?order_id=$orderID&customer_id=${Get.find<UserController>()
