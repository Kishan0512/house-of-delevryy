...
class OrderController extends GetxController implements GetxService {
  ...
  Future<void> placeSubscribeOrder(PlaceSubscribeBody placeSubscribeBody, Function callbackSubscribe, double amount) async {
    _isLoading = true;
    update();
    // print('JSON PRINT');
    ...
    if (response.statusCode == 200) {
      String message = response.body['message'];
      String orderID = response.body['order_id'].toString();
      callbackSubscribe(true, message, orderID, amount);
      // print('-------- Order placed successfully $orderID ----------');
    } else {
      callbackSubscribe(false, response.statusText, '-1', amount);
    }
    update();
  }
